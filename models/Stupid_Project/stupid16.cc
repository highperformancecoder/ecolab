		  ...
		  double max_food_production)
{
  ...
  for (int i=0; i<nx; i++)
	...
	Cell& cell=*getCell(o);
	cell.bugNbrhd=cell;
	// wire up predator neighbourhood - use Moore neighborhood, even though specification 
	// is ambiguous
  ...
  if (nbr!=neighbourlist.end())
    {
      ...
      if (newCell->predator.size()==0) // move only if destination is empty
	move(*newCell,*myCell);
   }
