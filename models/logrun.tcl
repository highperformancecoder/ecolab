...
#PBS -A rks -q checkable -lnodes=2:ppn=2 -lwalltime=3:0:0 
#\
ulimit -c unlimited
#\
cd $PBS_O_WORKDIR
#\
exec time lrun jellyfish logrun.tcl 

set startclock [clock seconds]
puts "Started at [clock format $startclock]"
...
# set the number of cells in each direction for the jellyfish neighbourhood map
    parallel nmapx [expr int([width] / ([speed_dist.scale]*5))]
    parallel nmapy [expr int([height] /([speed_dist.scale]*5))]
...
restart lake10.ckpt
puts stdout "cputime=[cputime]"
parallel jelly_map.Distribute_Objects
puts stdout "cputime=[cputime]"
parallel back_map.tag 0
lake.update_map
